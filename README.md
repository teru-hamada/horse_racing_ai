...
├─ .github/workflows/
│  ├─ daily-racing.yml      定期・手動の予想、前日照合、公開ファイルの登録
│  └─ deploy-pages.yml                    Pages配信とジョブ実行状況の生成
├─ src/
...
ローカルで生成したHTMLを公開する場合は、`docs/`の変更をコミットして`main`へpushします。`予想サイト配信・実行状況更新`が配信します。日次ジョブでは全レース検証後に公開用CSVも生成します。GUIでHTMLを生成するだけでは、日次ジョブの前日照合に必要な公開用CSVは揃いません。
...
`日次予想・結果照合・公開`が、開催確認、全レースのHTML取得、隔離DBへの登録、学習済みモデルによる予想、オッズ照合、買い目計算を行います。前日が開催日なら公開済みの予想・買い目を確定結果と照合します。ジョブ内でモデル学習は行いません。
...
1. **Actions → 日次予想・結果照合・公開 → Run workflow**を開きます。
2. デフォルトブランチを選択します。
...
取消・除外・中止・失格は数値の着順とは別に状態を保存し、状態を確認できる場合は照合を続けます。
取消・除外に伴う返還はJRAの公式返還情報で判定し、100円当たり100円を回収額へ反映します。
枠連は返還枠番と同枠のみの返還を区別し、競走中止を理由に返還扱いにはしません。
着順・状態が不明な場合や、取消・除外馬と公式返還馬番が一致しない場合は公開を止めます。
返還などを推測して払戻額を補いません。
...
公開サイトの**ジョブ実行状況**には、最新7件までの実行日時（日本時間）・結果（成功／失敗／中止等）・予想レース数・レース結果照合数を表示し、エラー詳細は掲載しません。レース数は各処理が完了した件数で、非開催・照合未指定は0、件数の記録がない過去の実行や取得できない記録は「—」になります。成功には非開催日の正常終了を含みます。記録がない日は一覧上で実行を確認できない日であり、未実行とは断定しません。

表示はPages配信時点の記録です。予想ワークフロー終了後は失敗・中止・非開催日でも`予想サイト配信・実行状況更新`が更新を試みます。実行履歴のAPI取得失敗を空の履歴として公開しません。履歴取得や配信自体に失敗した場合は公開済みの表示が残るため、ページの更新日時とActionsの結果を確認してください。
...
ジョブ実行状況用の件数は、別のArtifact `daily-racing-counts-<実行ID>-<試行回数>` 内の
`race_counts.json`へ保存します。こちらの保存期間は90日間で、予想レース数と結果照合数のみを含みます。
...
push後は`deploy-pages.yml` を呼び出して、確定したコミットのPages公開を直接実行します。
...
1レース予想・HTML取得・DB登録のPythonコマンドは診断用としても使用できます。
